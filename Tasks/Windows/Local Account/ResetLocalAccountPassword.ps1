<# 
  ...
  #>
Param (
$users = ""                    # List the scripts parameters here
)
...
$Global:ScriptParameters = "$users"                    # List the scripts parameters here as variables such as $param0 $param1 $param2, if none, leave as ""
$Global:Pass = "Password1!"                    # The password to reset the account to
$Global:UserCount = 0
$Global:ResetSuccess = 0
$Global:ExpireCount = 0
    # Log file path
...
Log-Start
if ($users -eq "") {    
    Log-Error -Messege "You havn't given me any usernames to reset..."
    } 
foreach($user in $users) {          # For each user in parameters
    $Global:UserCount++
    Log-Write -Messege "Checking if account exists for '$user'"
    $Checklocaluser = Check-LocalUserExist "$user"
    if ("$Checklocaluser" -eq "False") {          # If the user NOT exists, WARN
        Log-Write -status 1001 -Catagory "WARNING" -Messege "Cannot reset password for '$user', account doesn't exist"
        } elseif ("$Checklocaluser" -eq "True") {           # Elseif the user does exist, set password
            $CheckExpires = LocalUserFlags -flag 13 -type "Check" -user "$user"
            if ($CheckExpires -eq "True") {          # If password is set to never expire
                Log-Write -Messege "The user account '$user' is set to never expire, changing that now..."
                $RemoveExpire = LocalUserFlags -flag 13 -type "Remove" -user "$user"
                } else {
                    Log-Write -Messege "User account '$user' is already set to expire"
                    }
            Log-Write -Messege "Resetting password for user account '$user'"
            $objUserAccount = [ADSI]“WinNT://$env:COMPUTERNAME/$user”
            $objUserAccount.SetPassword("$Global:Pass")
            $objUserAccount.commitChanges()
            Log-Write -Messege "Verifying new password..."
            $CheckCred = Check-LocalUserCredential -UserName "$user" -Password "$pass"
            if ($CheckCred -eq "True") {          # If password was set
                Log-Write -Messege "Password for '$user' has successfully been reset"
                $Global:ResetSuccess++
                Log-Write -Messege "Expiring password for user account '$user'"
                $objUserAccount.PasswordExpired = 1
                $objUserAccount.commitChanges()
                $objUserAccount2 = [ADSI]“WinNT://$env:COMPUTERNAME/$user”
                if ($objUserAccount.PasswordExpired -ne 1) {
                    Log-Write -Messege "Failed to expire password for user account '$user'"
                    } else {
                        Log-Write -Messege "Successfully expired password for user account '$user'"
                        $Global:ExpireCount++
                        }
                } else {          # Elseif password was not set
                    Log-Write -Status 1001 -Catagory "WARNING" -Messege "Failed to reset password for user account '$user'"
                    }
            }
    }
if ("$Global:ResetSuccess" -eq "$Global:UserCount" -and $Global:ExpireCount -eq "$Global:UserCount") {
    Log-Write -Messege "$Global:ResetSuccess password(s) reset and $Global:ExpireCount password(s) expired out of $Global:UserCount"
    Log-Finish
    } else {
        Log-Error -Messege "$Global:ResetSuccess password(s) reset and $Global:ExpireCount password(s) expired out of $Global:UserCount"
        }
}

Catch {
Log-error -Messege "Passwords successfully reset = $Global:ResetSuccess : Passwords successfully expired = $Global:ExpireCount : Number of user accounts tried to reset = $Global:UserCount"
Write-Host "catch"
    }